...
@app.route("/api/public/trains/<loco_id>/stream")
def dashcam_stream_info(loco_id):
    if not is_valid_id(loco_id): abort(400, "Invalid ID")
    train_id = request.args.get('tid')
    
    if train_id and is_valid_id(train_id):
        try:
            r = requests.get(f"{BASE_API}/public/trains/{train_id}/stream", timeout=5)
            if r.status_code == 200:
                data = r.json()
                DASHCAM_CACHE[loco_id] = data
                return jsonify(data)
        except Exception: pass
        
    if loco_id in DASHCAM_CACHE:
        return jsonify(DASHCAM_CACHE[loco_id])
